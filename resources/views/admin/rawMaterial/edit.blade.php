            ...
            <div class="col-lg-12 col-md-12 col-sm-12">
                <div class="card border-success mb-3">
                    <h5 class="card-header bg-transparent border-default"><i class=" fa fa-refresh">&nbsp;</i> Update Raw Material</h5>
                        <form action="{{route('admin.rawMaterial.update',$rawMat->id)}}" method="POST" class="form-control">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <h6>Raw Material Name</h6>
                                <input type="text" name="name" placeholder="Name Here" class="form-control" value="{{$rawMat->name}}">
                            </div>
                            <div class="form-group">
                                <h6>Raw Material Description</h6>
                                <textarea class="form-control" style="height:30ch" name="description" placeholder="Description Here">{{$rawMat->description}}</textarea>
                            </div>
