            ...
            {{-- <img class="img-fluid mr-4" src="{{ asset('storage/forum_images/'.$forum->image)}}" alt="image" style="width:100%">
             ...
             <div class="card mb-3">
                    ...
                    <p class="card-text">{{$forum->body}}</p>
                        ...
                        <form action="{{route('user.comment.store')}}" method="POST" id="comment">
                            @csrf
                        ...
                        <ul class="list-group">
                            @foreach ($forum->comment->reverse() as $comment)
                                    <div class="media form-control">
                                        ...
                                        <div class="media-body">
                                            ...
                                            <!-- DELETE POST FUNCTION-->
                                                <form id="delete-{{$comment->id}}" action="{{route('user.comment.destroy',$comment->id)}}" method="POST" style="display:none">
                                                    @csrf
                                                    @method('DELETE')
                                                </form> 
                                            @endif
                                    ...
                                    <!---------MODAL FUNCTION HERE--->
                                    <form action="{{route('user.comment.update',$comment->id)}}" method="POST"> @csrf @method('PUT')
