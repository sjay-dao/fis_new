...
<h3><center>Productions&nbsp;&nbsp;Report<br>Total: {{$data->count()}}</center></h3>
          <table cellpadding="0" cellspacing="0" border="0">
            <thead style="background-color:rgb(232, 71, 27);">
              <tr>
                ...
                <th style="border:1 px solid black; color:white; "><center>Production Date</center></th>
                {{-- <th style="border:1 px solid black; color:white; "><center>Raw Material used</center></th> --}}
              </tr>
            </thead>
          </table>
          <table cellpadding="0" cellspacing="0" border="0">
            <tbody>
              @foreach ($data as $key=>$item)
                ...
                <td style="border:1 px solid black; width: 20%;">{{strtoupper($item->name)}}</td>
                    <td style="border:1 px solid black">{{$item->packs}}</td>
                    <td style="border:1 px solid black">{{date('F d, Y', strtotime($item->dateProduced))}}</td>
                    {{-- <td style="border:1 px solid black">
                      @if ($item->RawMaterialused->count() <= 0)
                          N/A
                      @endif
                    @foreach ($item->RawMaterialUsed as $key=>$rawmat)
                        {{$key+1}}.{{$rawmat->AdoptorRawmaterial->rawMaterial->name}}<br>
                    @endforeach --}}
                    </td>
